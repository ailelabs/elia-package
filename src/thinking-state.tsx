 ...
 * API: rows/query/activeLabel/doneLabel override the variant
 * presets; `working` makes it controlled (no scripted timeline);
 * defaultExpanded + onRowClick(row) for Search/Coding rows.
 * ───────────────────────────────────────────────────────── */
...
type Row = ThinkingRow;

export interface ThinkingStateProps {
  /** Unknown values fall back to Steps. */
  variant?: "Steps" | "Reasoning" | "Search" | "Coding" | (string & {});
  /** Trace rows. When provided, the scripted demo timeline is skipped. */
  rows?: ThinkingRow[];
  /** Search query line (demo mode falls back to the variant preset). */
  query?: string;
  /** Header label while working / when done. */
  activeLabel?: string;
  doneLabel?: string;
  /** Controlled working flag — when provided, honored instead of the script. */
  working?: boolean;
  /** Initial expanded state when not running the scripted demo. */
  defaultExpanded?: boolean;
  /** Fired when a Search or Coding row is clicked. */
  onRowClick?: (row: ThinkingRow) => void;
}
...
export default function ThinkingState({
  ...
  const isDemo = rows === undefined && workingProp === undefined;
  const stage = useSequence(isDemo ? STAGES : []);
  const [manualExpanded, setManualExpanded] = useState<boolean | null>(null);
  const [selectedTool, setSelectedTool] = useState<number | null>(null);
  const v = VARIANTS[variant] ?? VARIANTS.Steps;
  const rowData = rows ?? v.rows;
  const queryText = query ?? (rows === undefined ? v.query : undefined);
  const autoExpanded = isDemo
    ? stage >= 1 && stage < 4
    : (defaultExpanded ?? workingProp ?? false);
  const expanded = manualExpanded ?? autoExpanded;
  const working = workingProp ?? (isDemo ? stage < 3 : false);
  const visible = !isDemo
    ? rowData.length
    : stage < 2 ? 0 : stage === 2 ? Math.min(2, rowData.length) : rowData.length;
          ...
          <div className="relative mt-1 ml-[5px] pl-4">
            ...
            {queryText && (
              <div className="flex h-6 items-center gap-2 px-1.5" style={{ animation: expanded ? "fade-up 300ms cubic-bezier(0.23,1,0.32,1) both" : undefined }}>
                ...
                <span className="text-[12.5px] text-ink-2">{queryText}</span>
              </div>
            )}
            {rowData.slice(0, visible).map((row, i) => {
              const content = (
                <>
              ...
              if (variant === "Coding") {
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      setSelectedTool(selected ? null : i);
                      onRowClick?.(row);
                    }}
