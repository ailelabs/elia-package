 ...
 * API: workspace, items, sections, actionLabel, onSelect, onAction, onItemAdd.
 * ───────────────────────────────────────────────────────── */
...
export interface SidebarNavItem {
  ...
  label: string;
  section: string;
  /** Custom icon; falls back to the built-in icon matching `key`. */
...
export interface SidebarNavProps {
  ...
  onAction?: () => void;
  onItemAdd?: (key: string) => void;
}

const ITEMS: SidebarNavItem[] = [
  { key: "activity", label: "Home", section: "Workspace" },
  { key: "tasks", label: "Agent tasks", section: "Workspace", count: 4 },
...
export default function SidebarNav({
  workspace = { initial: "C", name: "Creamery Ops", sub: "Production Workspace" },
  items: itemsProp,
  ...
  const isDemo = itemsProp === undefined;
  const items = itemsProp ?? ITEMS;
  const [active, setActive] = useState(isDemo ? "tasks" : items[0]?.key ?? "");
  const [hovered, setHovered] = useState<string | null>(null);
  const [box, setBox] = useState<{ top: number; height: number } | null>(null);
  const [query, setQuery] = useState("");
  const [badge, setBadge] = useState(4);
  const sections = sectionsProp ?? [...new Set(items.map((item) => item.section))];
          ...
          <div key={section}>
            ...
            <div className="flex flex-col gap-px">
              {items.filter((item) => item.section === section).map((item) => {
                ...
                const count = isDemo && item.count !== undefined ? badge : item.count;
                return (
                  <button
                    key={item.key}
                    ref={(el) => {
