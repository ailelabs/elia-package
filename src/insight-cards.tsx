 ...
 * carousel, paged with the ‹ › controls.
 * API: pages ({key, prose, content, pill}), onPillClick, onPageChange;
 * Entity/Mono exported for composing prose.
 * ───────────────────────────────────────────────────────── */
...
export function Entity({ name, tone }: { name: string; tone: string }) {
  return (
...
export function Mono({ children, tone }: { children: React.ReactNode; tone: "red" | "green" }) {
  return (
...
export interface InsightPage {
  ...
  /** Rendered in place of the built-in card. */
...
export interface InsightCardsProps {
  pages?: InsightPage[];
  onPillClick?: (pageKey: string) => void;
...
const PAGES: InsightPage[] = [
  ...
  },
  {
    ...
    content: <AllocationCard />,
    pill: "If we look at seasonals, what changes?",
  },
];

export default function InsightCards({ pages = PAGES, onPillClick, onPageChange }: InsightCardsProps = {}) {
  ...
  const { key, prose, content, pill } = pages[page] ?? pages[0];
