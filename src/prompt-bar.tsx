 ...
 * API: sources, commands, models, files, onSend, onModelChange,
 * onConnect, autoplay (defaults on only when no data props given).
 * ───────────────────────────────────────────────────────── */
...
export type Source = {
  ...
  desc: string;
  /** Built-in glyph name. */
  glyph?: "clip" | "chart" | "layers" | "globe";
  /** Built-in brand mark. */
  brand?: "figma" | "slack" | "gmail";
  /** Picking this row attaches a file instead of inserting a mention. */
  attach?: boolean;
  /** Shows a Connect / Connected toggle on the row. */
  connect?: boolean;
};

export type Command = { key: string; name: string; desc: string };
...
export interface PromptBarProps {
  ...
  sources?: Source[];
  commands?: Command[];
  /** First model is the flagship — selecting it plays the rainbow sweep. */
  models?: Model[];
  /** File names cycled through by the attach row. */
  files?: string[];
  onSend?: (payload: { text: string; attachments: string[]; model: string }) => void;
  onModelChange?: (modelKey: string) => void;
  onConnect?: (sourceKey: string, connected: boolean) => void;
  /** Self-running walkthrough. Defaults to true only when no data props are passed. */
  autoplay?: boolean;
}
...
export default function PromptBar({
  ...
  /* demo mode: no data props passed (defaults are the module constants) */
  const isDemo =
    sources === SOURCES && commands === COMMANDS && models === MODELS && files === FILES;
  ...
  const [auto, setAuto] = useState(autoplay ?? isDemo);
   ...
   * Scripted steps mutate state directly — consumer callbacks never fire. */
  useEffect(() => {
    if (!auto) return;
    ...
    if (step.connect !== undefined)
      setConnected(
        step.connect
          ? Object.fromEntries(sources.filter((s) => s.connect).map((s) => [s.key, true]))
          : {},
      );
   ...
   * the fake transcript is demo-only; with real data the mic just stops */
  useEffect(() => {
    if (!listening) return;
    const t = setTimeout(() => {
      if (isDemo) {
  ...
  const send = () => {
    if (!canSend) return;
    onSend?.({ text: draft.trim(), attachments, model: model.key });
    setDraft("");
    setAttachments([]);
    closeMenus();
  };
      ...
      {menu && (
        ...
        >
          ...
          {rows.map((row, i) => {
            ...
            const isConnected = !!connected[row.key];
            return (
              <button
                key={row.key}
                type="button"
                ...
                {source?.connect && (
                  <span
                    ...
                    onClick={(event) => {
                      ...
                      setConnected((current) => ({ ...current, [row.key]: !isConnected }));
                      onConnect?.(row.key, !isConnected);
                    }}
                    className={`shrink-0 text-[12px] font-medium transition-colors duration-100 ${
                      isConnected ? "text-green" : "text-accent-ink hover:underline"
                    }`}
                  >
