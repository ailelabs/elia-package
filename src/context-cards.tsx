 ...
 * API: chunks?, totalCount?, onSourceClick?(chunk) — zero-config renders the demo.
 * ───────────────────────────────────────────────────────── */
...
export interface ContextCardsProps {
  /** Retrieved chunks to display. Omit to render the built-in demo. */
  chunks?: ContextChunk[];
  /** Count shown in the header badge. Defaults to 32 in demo, chunks.length otherwise. */
  totalCount?: number;
  /** Fired when a source chip is clicked. */
  onSourceClick?: (chunk: ContextChunk) => void;
}
...
export default function ContextCards({
  ...
  const isDemo = chunks === undefined;
  const data = chunks ?? CHUNKS;
  const count = totalCount ?? (isDemo ? 32 : data.length);
  const [chipsShown, setChipsShown] = useState(!isDemo);
      ...
      {data.map((chunk, i) => (
        ...
        >
          ...
          <div className="px-3 pb-3">
            ...
            >
              <span className={`flex size-3.5 items-center justify-center rounded-[4px] ${chunk.tone ?? "bg-accent"} text-[7px] font-bold text-white`}>
                {chunk.badge}
              </span>
              {chunk.source}
