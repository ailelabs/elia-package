 ...
 * API: questions? + onSubmit?(answers) + onDismiss?().
 * ───────────────────────────────────────────────────────── */
...
export interface ApprovalCardProps {
  ...
  onSubmit?: (answers: Record<number, ApprovalAnswer>) => void;
  onDismiss?: () => void;
}
...
export default function ApprovalCard({
  ...
  const submit = (a: Record<number, number[]>, c: Record<number, string>) => {
    setSent(true);
    if (!onSubmit) return;
    const payload: Record<number, ApprovalAnswer> = {};
    questions.forEach((item, i) => {
      const trimmed = c[i]?.trim();
      payload[i] = {
        selected: (a[i] ?? []).map((index) => item.options[index]),
  ...
  const toggle = (index: number) => {
    if (question.type === "radio") {
      const nextAnswers = { ...answers, [qi]: [index] };
        ...
        <div className="primitive-card-footer flex items-center justify-between">
          ...
          {!sent && (
            <button
              ...
              disabled={!hasAnswer}
              onClick={() => last ? submit(answers, custom) : setQi((current) => current + 1)}
