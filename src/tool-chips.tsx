 ...
 * API: header, rows, diffs, moreCount — staggered demo
 * reveal only runs when rows is omitted.
 * ───────────────────────────────────────────────────────── */
...
export default function ToolChips({
  ...
  const diffData = diffs ?? (isDemo ? DIFFS : []);
  const more = moreCount ?? (isDemo ? 2 : 0);
  const [step, setStep] = useState(0);
  const [open, setOpen] = useState(true);
  const [openRows, setOpenRows] = useState<Set<number>>(new Set());
  const total = data.length + 1; // rows, then diff chips
  const visible = isDemo ? step : data.length;
  const showDiffs = !isDemo || step >= total;

  useEffect(() => {
    if (!isDemo || step >= total) return;
    ...
    <div className="min-h-[220px] w-full max-w-80 pb-1">
      ...
      <div className="grid transition-[grid-template-rows,opacity] duration-300" style={{ gridTemplateRows: open ? "1fr" : "0fr", opacity: open ? 1 : 0 }}>
        ...
        <div className="mt-1.5 flex flex-col gap-1">
          {data.slice(0, visible).map((row, i) => {
            const rowOpen = openRows.has(i);
            return (
            <div key={i} style={{ animation: "fade-up 300ms cubic-bezier(0.23,1,0.32,1) both" }}>
              <button
                type="button"
      ...
      {/* file-diff chips */}
      {showDiffs && (diffData.length > 0 || more > 0) && (
        <div className="mt-2.5 flex max-w-full flex-wrap gap-1.5 border-t border-line pt-2.5">
          ...
          {more > 0 && (
            <button
              ...
              style={{ animation: `fade-in 300ms ease-out ${diffData.length * 80}ms both` }}
            >
              +{more} more
            </button>
          )}
