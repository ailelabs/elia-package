 ...
 * API: rows, tagColors, strengthLabels, onSelectionChange, onSortChange.
 * ───────────────────────────────────────────────────────── */

export type RecordsStrength = "strong" | "weak" | "veryweak" | "none";
...
export const STRENGTH: Record<RecordsStrength, { label: string; color: string; rank: number }> = {
...
export interface RecordsRow {
  ...
  tags: string[];
  last: string;
  strength: RecordsStrength;
  website?: string;
}

export interface RecordsTableProps {
  rows?: RecordsRow[];
  /** Merged over the built-in tag palette. */
  tagColors?: Record<string, string>;
  strengthLabels?: Partial<Record<RecordsStrength, string>>;
  onSelectionChange?: (ids: string[]) => void;
  onSortChange?: (sort: RecordsSort) => void;
}

const INITIAL_ROWS: RecordsRow[] = [
...
function Tag({ name, colors }: { name: string; colors: Record<string, string> }) {
  const color = colors[name] ?? "#7f858d";
  return (
...
function HeaderCell({ label, icon, sortKey, sort, onSort, className = "" }: { label: string; icon: React.ReactNode; sortKey?: RecordsSortKey; sort: RecordsSort; onSort: (key: RecordsSortKey) => void; className?: string }) {
  return (
...
export default function RecordsTable({ rows = INITIAL_ROWS, tagColors, strengthLabels, onSelectionChange, onSortChange }: RecordsTableProps = {}) {
  const [selected, setSelected] = useState<Set<string>>(new Set());
  const [sort, setSort] = useState<RecordsSort>({ key: "name", dir: 1 });
  const colors = useMemo(() => ({ ...TAG_COLORS, ...tagColors }), [tagColors]);
  const strengthLabel = (key: RecordsStrength) => strengthLabels?.[key] ?? STRENGTH[key].label;
  ...
  const toggleSort = (key: RecordsSortKey) => {
    const next: RecordsSort = sort.key === key ? { key, dir: (sort.dir * -1) as 1 | -1 } : { key, dir: 1 };
    setSort(next);
    onSortChange?.(next);
  };
          ...
          <tbody>
            {visibleRows.map((row) => {
              ...
              return <tr key={row.id} className={`records-row ${selectedRow ? "is-selected" : ""}`}>
                ...
                <td className="records-cell"><div className="records-tags">{row.tags.slice(0, 4).map((tag) => <Tag key={tag} name={tag} colors={colors} />)}{row.tags.length > 4 ? <span className="records-more-tag">+{row.tags.length - 4}</span> : null}</div></td>
                <td className={`records-cell ${row.last === "No contact" ? "records-muted" : ""}`}>{row.last}</td>
                <td className="records-cell"><span className="records-strength"><span className="records-strength-dot" style={{ background: strength.color }} />{strengthLabel(row.strength)}</span></td>
