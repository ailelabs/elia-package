 ...
 * API: items?, placeholder?, emptyTitle?/emptyDesc?, onSelect?(item), onQueryChange?(q).
 * ───────────────────────────────────────────────────────── */

export interface SearchListProps {
  /** Searchable items. Omit to render the built-in demo. */
  items?: string[];
  placeholder?: string;
  emptyTitle?: string;
  emptyDesc?: string;
  /** Fired when a result is clicked. */
  onSelect?: (item: string) => void;
  /** Fired on every query change (typing or clearing). */
...
export default function SearchList({
  ...
  emptyDesc = "Adjust your search to try again",
  onSelect,
  onQueryChange,
}: SearchListProps = {}) {
  ...
  const results = query
    ? data.filter((i) => i.toLowerCase().includes(query.toLowerCase()))
    : data.slice(0, 5);
      ...
      <div className="w-full self-start overflow-hidden rounded-card bg-surface shadow-raised">
        ...
        {empty ? (
          <div className="flex flex-col items-center justify-center gap-1 px-4 py-8" style={{ animation: "fade-in 250ms ease-out both" }}>
            ...
            <span className="text-[12px] text-ink-3">{emptyDesc}</span>
          </div>
        ) : (
          <div className="p-1">
            {results.map((item) => (
