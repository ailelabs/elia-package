 ...
 * API: text/sources/followUps replace the demo data;
 * onAction("copy"|"retry"|"up"|"down"), onFollowUp(text);
 * loop replays the stream (defaults on only in demo mode).
 * ───────────────────────────────────────────────────────── */
...
export interface StreamingSource {
  ...
  domain: string;
  href: string;
  /** Optional tile image URL — falls back to a generated initial tile. */
  image?: string;
}

export type StreamingAction = "copy" | "retry" | "up" | "down";

export interface StreamingTextProps {
  /** The answer text. When provided, the demo script is replaced. */
  text?: string;
  sources?: StreamingSource[];
  followUps?: string[];
  onFollowUp?: (text: string) => void;
  onAction?: (action: StreamingAction) => void;
  /** Replay the stream after a hold. Defaults to true only in demo mode. */
  loop?: boolean;
}

function sourceImage(source: StreamingSource) {
  if (source.image) return source.image;
  const initial = (source.name || source.domain || "?").trim().charAt(0).toUpperCase();
  return `data:image/svg+xml,${encodeURIComponent(
    `<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'><rect width='64' height='64' rx='16' fill='#5c6470'/><text x='32' y='43' text-anchor='middle' font-family='system-ui,sans-serif' font-size='30' font-weight='600' fill='#fff'>${initial}</text></svg>`,
  )}`;
}
...
const ACTION_NAMES: StreamingAction[] = ["copy", "retry", "up", "down"];
...
export default function StreamingText({
  ...
  const isDemo = text === undefined;
  const tokens: Token[] = isDemo
    ? TOKENS
    : text.split(" ").map((word) => ({ text: word }));
  const sourceList = sources ?? (isDemo ? SOURCES : []);
  const followUpList = followUps ?? (isDemo ? FOLLOW_UPS : []);
  const shouldLoop = loop ?? isDemo;
      ...
      >
        {ACTION_ICONS.map((icon, i) => (
          <button
            key={i}
            type="button"
            aria-label={ACTION_NAMES[i]}
            onClick={() => onAction?.(ACTION_NAMES[i])}
        ...
        {sourceList.length > 0 && (
          ...
          >
            ...
            <span className="text-[12px] text-ink-2">
              {sources ? `${sourceList.length} sources` : "10 sources"}
            </span>
          </button>
        )}
      </div>
