...
import { StreamText } from "./atoms/stream-text";
 ...
 * tokens.
 * API: lead?/picked?/rewrite? + actions? +
 * onRun?(action, prompt) + onKeep?() + onDiscard?().
 * ───────────────────────────────────────────────────────── */
...
const ACTIONS: SelectionActionKey[] = [
  "explain",
  "improve",
  "shorten",
  "tone",
...
export default function SelectionActions({
  ...
  const run = (nextAction: string) => {
    onRun?.(nextAction, prompt.trim());
    setAction(nextAction);
    setExpanded(false);
    setMode("thinking");
  };

  const has = (key: SelectionActionKey) => actions.includes(key);
  const hasMore = has("shorten") || has("tone") || has("grammar");
              ...
              <>
                ...
                >
                  ...
                  {has("explain") && (
                    <button
                      type="button"
                      onClick={() => onRun?.("Explain", prompt.trim())}
                      className={control}
                    >
                      {icons.explain}
                      Explain
