 ...
 * API: fileName, language, code (plain) | lines (tokens),
 * stream, onCopy — the looping demo only runs when both
 * code and lines are omitted.
 * ───────────────────────────────────────────────────────── */

const LINE_MS = 240;
const HOLD_MS = 3200;

export type CodeToken = { t: string; c?: "kw" | "str" | "num" | "fn" | "dim" };
type Tok = CodeToken;

export interface CodeBlockProps {
  fileName?: string;
  language?: string;
  /** Plain source — rendered untokenized. */
  code?: string;
  /** Pre-tokenized lines for callers who want colors; wins over `code`. */
  lines?: CodeToken[][];
  /** Stream consumer code line-by-line once. Demo mode always streams. */
  stream?: boolean;
...
export default function CodeBlock({
  ...
  const isDemo = code === undefined && lines === undefined;
  const tokenLines: Tok[][] =
    lines ?? (code !== undefined ? code.split("\n").map((t) => [{ t }]) : LINES);
  const raw = code ?? (lines ? lines.map((l) => l.map((t) => t.t).join("")).join("\n") : RAW);
  const streaming = isDemo || stream === true;
  const [count, setCount] = useState(0);
  const [copied, setCopied] = useState(false);
  const shown = streaming ? count : tokenLines.length;
  const done = shown >= tokenLines.length;

  useEffect(() => {
    if (!streaming || (!isDemo && done)) return;
    const t = setTimeout(
      () => setCount((c) => (c >= tokenLines.length ? 0 : c + 1)),
      count === 0 ? 400 : done ? HOLD_MS : LINE_MS,
    );
    return () => clearTimeout(t);
  }, [count, done, streaming, isDemo, tokenLines.length]);
  ...
  return (
    <div className="w-full max-w-95 overflow-hidden rounded-card bg-surface shadow-card">
      ...
      <pre className="min-h-[137px] bg-inset px-3 py-2.5 font-mono text-[11.5px] leading-[1.7]">
        {tokenLines.slice(0, shown).map((line, i) => (
          <div
            key={i}
            className="flex"
